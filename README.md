...
Target Users
System Owners/Admins: Full system access, user management, department oversight
Project Managers: Manage projects within their department, assign tasks
Team Members: Work on assigned tasks, create subtasks, collaborate
Collaborators: External users with limited access to shared projects
External Viewers: Public access via shareable links (read-only)
Feature Breakdown
...
Creator automatically becomes Project Head (owner_id)
Project is assigned to a department (user's department or selected)
A default TaskList is auto-created
Creator has full control: can invite members, assign roles, manage tasks
Task Assignment Workflow
Project Head or PM creates a root task
Task can be assigned to any user (cross-department allowed)
Assigned user receives:
...
Authorized users can create subtasks under any task
Subtasks inherit project and task list from parent
Subtasks can have their own assignees (independent of parent)
Subtasks can be nested recursively (unlimited)
Parent task completion doesn't auto-complete subtasks (explicit action required)
Sharing Workflow
Public Sharing:
...
Project Channel Subscription (useRealTime.ts):
...
Route Organization
Public Routes: /api/public-shares/{token} (no auth)
Auth Routes: /api/auth/* (login, register, profile)
Dashboard Routes: Role-based dashboards
Admin Routes: /api/admin/* (owner only)
...
Select Specific Columns: Reduce memory usage
...
Login: Sarah logs in with her credentials
Navigate to Projects: Clicks "Projects" in sidebar
Create Project:
Clicks "New Project"
...
Login: Mike logs in
View My Tasks: Clicks "My Tasks" in sidebar
...
Receives Invitation: Email notification with project share link
Login: Logs in (or creates account)
...
Receives Link: Gets public share link via email/Slack
Opens Link: /public/projects/{token}
...
Cannot Edit: All edit actions disabled (read-only)
Technical Flow:
...
Login: Alex logs in with team_member role
Create Project:
...
Invite Cross-Department Members:
...
Create Another Cross-Department Task:
